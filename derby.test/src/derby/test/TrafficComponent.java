		...
		try {
			...
			for (EObject eo : loadDetectorResource.getContents()) {
				...
				if (eo instanceof EPushStreamProvider) {
					...
					epsp.createPushStream().forEach((e)->System.out.println("PSP Loaded: " + e.toString()));
				}
			}
			System.out.println("Loaded data: " + loadDetectorResource.getContents().size() + " in " + (System.currentTimeMillis() - start) + " ms");
		} catch (IOException e) {
